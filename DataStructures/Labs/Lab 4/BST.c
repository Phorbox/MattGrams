...
/******************** ceiling **************************************
Purpose:
 * finds the ceiling of the average of two numbers, typically used with assembling a preconstructed BST in function *build.
Parameters:
 *     int left, int right: the two numbers to be averaged
Returns:
 * if sum of parameters is even, then return the average, if odd, returns average +1
**************************************************************************/
int ceiling(int left, int mid)
{
    if ((left + mid) % 2 == 1)
    {
        return ((left + mid) / 2) + 1;
...
nodeT *build(int left, int mid, int right)
{
    ...
    pRoot->pLeft = build(left, ceiling(left, mid), mid);
